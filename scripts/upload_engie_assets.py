...
def main():
    ...
    # The user said @[DATA/results/engie_asset_mapping_final.csv] is at /home/boujuan/Documents/ZENTUS/ERCOT/DATA/results/engie_asset_mapping_final.csv
    ...
    for _, row in df.iterrows():
        ...
        mwh = pd.to_numeric(row.get('Nameplate Energy Capacity (MWh)'), errors='coerce')
        
        # Handle NaN from coercion
        if pd.isna(mw): mw = None
        if pd.isna(mwh): mwh = None
        ...
        record = {
            ...
            'operating_year': int(row.get('Operating Year')) if pd.notna(row.get('Operating Year')) else None,
            'resource_name': row.get('resource_name'),
            'asset_id': row.get('asset_id'),
            'hsl': pd.to_numeric(row.get('hsl'), errors='coerce'),
            'lsl': pd.to_numeric(row.get('lsl'), errors='coerce')
        }
